...
for pi = 1:length(templates)
    ...
    disp('Features of Letter : '+All_letters(pi));
    ...
    %[~, numberOfClosedRegions] = bwlabel(1-(cell2mat(templates(pi))));
     ...
     Maj_Axis(pi)=stats.MajorAxisLength;
     Min_Axix(pi)=stats.MinorAxisLength;
     peri(pi)=stats.Perimeter;
     
     A(pi,1)=All_letters(pi);
     A(pi,2)=area(pi);
     A(pi,3)=centsx(pi);
     A(pi,4)=centsy(pi);
     A(pi,5)=Maj_Axis(pi);
     A(pi,6)=Min_Axix(pi);
     A(pi,7)=filled_area(pi);
     A(pi,8)=peri(pi);
     
 %xlswrite('test.xls',A(pi,2))    
end
...
%figure
%plot(Min_Axix,Maj_Axis,'.m')
%text(Min_Axix,Maj_Axis,All_letters,'VerticalAlignment','bottom','HorizontalAlignment','right')
%xlim([20 50])
%ylim([35 65])
%title('Major and Minor axis')
...
x=(1:36);
plot(x,Maj_Axis,'*c')
text(x,Maj_Axis,All_letters,'VerticalAlignment','bottom','HorizontalAlignment','right')
%ylim([min(Maj_Axis)-50 max(Maj_Axis)+50])
ylim([40 60])
title('MajorAxisLength value of all Characters')
...
x=(1:36);
plot(x,Min_Axix,'*m')
text(x,Min_Axix,All_letters,'VerticalAlignment','bottom','HorizontalAlignment','right')
%ylim([min(Min_Axix)-20 max(Min_Axix)+20])
ylim([20 40])
title('MinorAxisLength value of all Characters')
...
x=(1:36);
plot(x,peri,'*b')
text(x,peri,All_letters,'VerticalAlignment','bottom','HorizontalAlignment','right')
%ylim([min(Maj_Axis)-50 max(Maj_Axis)+50])
ylim([100 275])
title('Perimeter - Value of all Characters')
